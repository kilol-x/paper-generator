...
<nav v-else><button :class="{on:page==='students'}" @click="page='students'">▦ 学生管理</button><button :class="{on:page==='papers'}" @click="page='papers'">▤ 论文管理</button><button :class="{on:page==='review'}" @click="page='review'">✎ 论文批阅</button></nav><div class="user">{{session.nickname||session.username}} · {{session.role}}</div></aside><main><header><h1>{{role==='admin'?'管理员系统':role==='student'?(page==='overview'?'我的论文':'教师批阅反馈'):(page==='students'?'学生管理':page==='papers'?'学生论文管理':'论文批阅工作台')}}</h1><div class="header-actions"><div v-if="role==='teacher'" class="state green">仅展示名下学生真实数据</div><button class="logout" @click="logout">退出登录</button></div></header><div class="content" :class="{flush:role==='admin'||(role==='student'&&page==='overview')}">
